...
class AssetTest extends AppTestCase
{
	...
	public static function setUpBeforeClass() {
		...
		foreach (self::$_files as $dirName => $dirFiles) {
			...
			foreach ($dirFiles as $fileName) {
				...
				file_put_contents(
					$fullFileName, '<script id="' . $fileName . '" type="text/x-handlebars-template"></script>'
				);
			}
		}
	}
	...
	public function testBadVarsInvalidNames() {
		...
		foreach ($invalidNames as $invalidName) {
			...
			self::assertEquals(
				"Невалидное название переменной '$invalidName'", $errorMsg,
	...
	public function testLoadParams() {
		...
		self::assertEquals(
			'Не определены обязательные переменные: testLoad', $errorMsg,
		...
		self::assertEquals(
			'Не определены обязательные переменные: testLoad', $errorMsg, 'Параметры не перевелись в camelCase'
		);
	}
	...
	public function testLoadAssetVars() {
		...
		self::assertEquals(
			'Не определены обязательные переменные: var6', $errorMsg,
		...
		self::assertEquals(
			'Конфликт переменных: var2 с типами json и bool', $errorMsg,
		...
		self::assertEquals(
			'var7 должна иметь тип num, а не bool', $errorMsg,
