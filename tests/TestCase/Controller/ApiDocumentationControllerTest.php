...
class ApiDocumentationControllerTest extends AppControllerTestCase
{
    ...
    public function test()
    {
        ...
        $jsonSchema = $this->getJsonResponse('/apiDocumentation.json');
    ...
    private function _testDefinition(array $definition, string $definitionPath, array $jsonSchema)
    {
        if (!empty($definition['$ref'])) {
            ...
            self::assertNotNull($checkObject, 'Не описаны свойства в ' . $definitionPath);
            ...
            self::assertNotEmpty($definition['properties'], 'Не описаны свойства в ' . $definitionPath);
    ...
    private function _testPaths(array $jsonSchema)
    {
        ...
        foreach ($jsonSchema['paths'] as $pathUrl => $path) {
            foreach ($path as $requestMethod => $pathElement) {
                ...
                self::assertTrue(!empty($pathElement['responses'][200]) || !empty($pathElement['responses'][302]));
                ...
                self::assertFalse(in_array($pathElement['operationId'], $operationIds), 'Не уникальной operationId для ' . $pathUrl . '::' . $requestMethod);
            }
        }
    }
    ...
    private function _checkParameter(array $parameter, string $parameterPath, array $jsonSchema)
    {
        if (empty($parameter['$ref'])) {
            ...
            if (!empty($parameter['schema'])) {
                ...
                self::assertNotNull($parameter['type'], 'Пустое свойство ' . $parameterPath . '::type');
            }
        } else {
    ...
    private function _checkForRequiredProperties(array $object, array $propertyList, string $errorMessage)
    {
        foreach ($propertyList as $propertyName) {
            self::assertTrue(isset($object[$propertyName]), $errorMessage . '::' . $propertyName);
        }
    }
    ...
    private function _checkReference(string $reference, string $propertyPath, array $jsonSchema)
    {
        ...
        self::assertTrue(Strings::startsWith($reference, '#/'), 'Внешние ссылки запрещены');
        ...
        foreach ($referenceArr as $pathElement) {
            self::assertTrue(isset($pathStep[$pathElement]) && !empty($pathStep[$pathElement]), $errorMessage);
